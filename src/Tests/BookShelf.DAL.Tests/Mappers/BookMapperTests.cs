...
public sealed class BookMapperTests
{
    ...
    [TestMethod]
    public void GIVEN_SearchBooksWithPaginationResult_WHEN_MapCalled_THEN_BookObjectCreated()
    {
        // Arrange
        var bookEntity = new SearchBooksWithPaginationResult
        {
            Id = Guid.NewGuid(),
            Title = "Book Title",
            Author = "Some author",
        ...
        result.Quantity.Should().Be(bookEntity.Quantity);
        result.CategoryId.Should().Be(bookEntity.CategoryId);
        result.CategoryName.Should().Be(bookEntity.CategoryName);
    }
}